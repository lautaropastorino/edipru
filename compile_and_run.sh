...
# $4 = nombre del package java (puede ser vacio)
# $5 = args
...
ARGS=()
if [ "0" != "$5" ]; then # Si hay args
    IFS=' '     # space is set as delimiter
    read -ra ARGS <<< "$5" # Es como $5.split(" ") ==> Guardo los args en un arreglo
fi
...
if [ "0" = "$4" ]; then # no hay package name
    run="java -cp /jade/lib/jade.jar:/jade/classes jade.Boot -container -host 192.168.50.4 -agents '$3:$nombre("
else # hay package name
    run="java -cp /jade/lib/jade.jar:/jade/classes jade.Boot -container -host 192.168.50.4 -agents '$3:$4.$nombre("
fi
